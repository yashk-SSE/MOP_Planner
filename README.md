...
## Historical tab
Shows the raw BQL/MS/MD/Order/HOTO and all 4 funnel rates for every trailing month, by city or by sub-channel — this is what feeds the trend calculation, laid out so any projected number can be traced back to its source months. In sub-channel view, a "Show channels" picker lets you toggle any of the 5 core channels plus BTL on/off (all on by default) — BTL is included here as an optional reference only; it never joins the actual projection anywhere else in the tool.
