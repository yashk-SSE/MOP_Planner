...
## Cities: real city-level variation, normalized to reconcile with Summary
Each city keeps its own historical rate shape (Pune ≠ Nagpur ≠ Ahmedabad), but every stage (BQL→MS, MS→MD, MD→Order, Order→HOTO) is scaled — multiplicatively, one stage at a time, using that stage's own volume as the weight — so the volume-weighted average across all 27 cities lands exactly on Pan-India's rate for that stage. This is the same principle BQL shares already use (city-level detail that adds up to the known total), just applied to rates too. As long as no individual city has its own direct override, the Cities total always matches Summary exactly on every metric. Overriding a specific city's rate or volume still works exactly as before and creates a visible, deliberate local deviation — the Cities total will then differ from Summary by exactly the amount attributable to that override, which is the point (it's no longer an unexplained mismatch). A city's own historical rate (pre-normalization) is still available for reference — it just isn't the number applied to the projection anymore.
...
A tab that dynamically renames itself to whichever month is your current in-progress reference month (e.g. "July Projection"). While that month is still in progress, its City-level and Sub-Channel-level BQL/MS/MD/Order/HOTO and rates are editable — corrections here replace the automatic seasonally-estimated figure everywhere that month feeds a trend calculation, not just in this tab. Persists across sessions (localStorage) until the month is fully complete, at which point real actuals take over automatically and the correction is simply no longer read. Both tables lead with a bolded total row ("India Total" / "Overall (all channels)") — a sum of what's shown, with rates re-derived from the summed volumes rather than averaged, same convention as the Historical tab. The two totals can differ very slightly from each other (a percent or so) since City-level and Sub-Channel-level are independently estimated from the same underlying data — expected small-sample noise, not a bug.
